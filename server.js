...
app.get("/micro-frontends-config", async function (req, res) {
  let mfeRoutes;
  var env_config = APP_CONST.APP_CONFIG.find(function (item) {
    return item.appEnv == process.env.APPENV.toLowerCase();
  });
  switch (process.env.APPENV.toLowerCase()) {
    ...
    case APP_CONST.APP_ENV_LOCAL_MULTI.toLowerCase():
      mfeRoutes = await fsPromises.readFile(
        path.join(configPath + env_config.mfeConfigPath)
      );
      break;
    default:
      res.send({
        error: { message: "Check application environment", code: 500 },
      });
      break;
  }
  res.send(mfeRoutes);
...
app.get("*", async function (req, res) {
  ...
  switch (process.env.APPENV.toLowerCase()) {
    ...
    case APP_CONST.APP_ENV_DEV.toLowerCase():
    case APP_CONST.APP_ENV_LOCAL.toLowerCase():
      mfeIndex = await fsPromises.readFile(
        path.join(distPath + env_config.mfeIndexPath)
      );
      break;
    case APP_CONST.APP_ENV_LOCAL_MULTI.toLowerCase():
  ...
  switch (process.env.APPENV.toLowerCase()) {
    ...
    case APP_CONST.APP_ENV_LOCAL_MULTI.toLowerCase():
      mfeRoutes = await fsPromises.readFile(
        path.join(configPath + env_config.mfeRoutesPath)
      );
      break;
    default:
      res.send({
  ...
  singleSpaRouter.insertAdjacentHTML(
    "beforeend",
